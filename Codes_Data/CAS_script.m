...
Y = 0.5; % [mg VSS/mg BOD] (Metcalf and Eddy, 2013, table 7-8, p. 593)
...
%     SRT = 0.5; % [d]
    S = K*((1+b*SRT)/(Y*q_hat*SRT-(1+b*SRT))); % [mg/L]
    HRT = (SRT/MLSS(i))*(X_inert+((1+(1-f_d)*b*SRT)/(1+b*SRT))*Y*(S_SO-S)); % [d]
%     HRT = 0.5/24; % [d]
    V = Q_LD*HRT; % [L]
    Q_was = (((MLSS(i)*V)/SRT)-(Q_LD*X_V_e))/(X_V_w-X_V_e); % [L/d]
    Q_ras = ((X_V_w*Q_was)+(X_V_e*(Q_LD-Q_was))-(MLSS(i)*Q_LD))/(MLSS(i)-X_V_w); % [L/d]
    X_a = ((SRT/HRT)*Y*(S_SO-S))/(1+b*SRT); % [mg VSS/L]
    r_ut = (S_SO-S)/HRT; % [mg/L-d]
    
    %SMP calc
    UAP = (-(q_UAP*X_a*HRT+K_UAP+k1*r_ut*HRT)+sqrt(((q_UAP*X_a*HRT+K_UAP+k1*r_ut*HRT)^2)+4*K_UAP*k1*r_ut*HRT))/2; % [mg COD/L]
    BAP = (-(K_BAP+(q_BAP-k2)*X_a*HRT)+sqrt(((K_BAP+(q_BAP-k2)*X_a*HRT)^2)+4*K_BAP*k2*X_a*HRT))/2; % [mg COD/L]
    SMP = UAP+BAP; % [mg COD/L]
    
    %Clarifier sizing
    A_Clarifier = ((Q_LD+Q_ras)*MLSS(i)*2.20462e-6)/SLR; % [ft^2], rectangular with floor slope of 1% (Clarifier Design, 2nd Ed., WEF, MOP FD-8), 2.20462e-6 converts mg to lb
    
    %Reactor sizing
    L_tank = (V*0.0353147)/(w*d); % [ft], 0.0353147 converts L to ft3
    ...
    %O2 requirement
    dMLSS_dt = (MLSS(i)*V)/SRT; % [mg VSS/d]
    ...
    [~, M_SS_blower, OD_gh, OD_gsm] = Gas_Sparging_Submerged(O2_cfm, L_tank, W_PB, L_BB);
    Power_reqd = (O2_uptake*(10^(-6)))/24; % [kW], assumes 1 kWh/1 kg O2, Rittmann and McCarty, 2001, pg. 352
    P_input_blower = Power_reqd;
    ...
    %Pumping
    P_input_LIFT = 0; % Eliminated lift pumping 11/20/15
    M_SS_LIFT = 0;
    %% AD
    ...
    Q_was_thickened_LD = (Q_was)*(X_V_w/X_thickened_WAS);
...
%     S_vol = S_mass/(1.02*10^3*0.05); % [m3/d], Assumes sludge is 95% moisture, has a specific gravity of 1.02, 10^3 = specific weight of water
    BOD_loading = AD_Q_in*X_thickened_WAS*1.42; % [g COD/d]
    Vol_loading = BOD_loading/(AD_V*1000); % [kg/m3-d]
    VSS_produced = (0.08*(BOD_loading-BOD_loading*(1-0.5))*(10^(-3)))/(1+0.03*AD_SRT); % [kg/d] 0.08 = Y, 0.03 = b, 0.7 = efficiency of waste conversion (Metcalf, pg 1508)
    Q_CH4 = 0.4*((BOD_loading-BOD_loading*(1-0.5))*(10^(-3))-1.42*VSS_produced); % [m3/d]
    Q_CH4_burned = 0;
    ...
    while Heating_total > heat
        ...
        [P_offset, heat] = CHP(Q_CH4);
        heat = heat + Q_CH4_burned*35846*1000;
    end
    ...
    %% Sludge handling
    Q_WAS_mgd = Q_was/3785411.78;
    Q_cmd = Q_mgd(i) * 3785.41178; % [m^3/day] Unit conversion
    M_COD_removed = (S_SO - S)/10^3 * Q_cmd; % [kg-COD/day] Daily COD removal
    ...
    L_MT = A_Clarifier/(w*N_train); % [ft]
    
    %% Concrete
    % Concrete [ft^3] - Distribution Channel
    W_dist = 4.5; % [ft] Width of distribution channel
    W_N_trains = (w + 2 * t_wall) * N_train - t_wall * (N_train - 1);
    VWC_dist = d * t_wall * (2 * W_N_trains + 2 * W_dist);
    VSC_dist = W_N_trains * (W_dist + 2 * t_wall)+ W_N_trains * (W_dist + 2 * t_wall) * t_slab;
    
    % Concrete [ft^3] - CSTR Trains
    VWC_CSTR = d * t_wall * (N_train + 1) * L_tank;
    VSC_CSTR = N_train * (d + 2.4 + 4.81 + 2.4 + 7.26) * L_tank +N_train * (d + 2.4 + 4.81 + 2.4 + 7.26) * t_slab * L_tank;
    
    % Concrete [ft^3] - Membrane Trains
    VWC_MT = d * t_wall * (N_train + 1) * L_MT;
    VSC_MT = N_train * (d + 2.4 + 4.81 + 2.4 + 7.26) * L_MT+N_train * (d + 2.4 + 4.81 + 2.4 + 7.26) * t_slab * L_MT;
    
    % Concrete [ft^3] - Effluent Channel
    W_eff = 5; % [ft] Width of effluent channel
    VWC_eff = d * t_wall * (2 * W_N_trains +  2 * W_eff);
    VSC_eff = W_N_trains * (W_eff + 2 * t_wall)+ W_N_trains * (W_eff + 2 * t_wall) * t_slab;
    
    % Pump/Blower Building
    VWC_PBB = d * t_wall * (2 * W_N_trains + 2 * W_PB + 2 * W_BB);
    VSC_PBB = W_N_trains * (W_PB + t_wall + W_BB)+W_N_trains * (W_PB + t_wall + W_BB) * t_slab;
    ...
    Area_B_train = (W_dist + L_MT + W_eff + 2 * CA) * (W_N_trains + 2 * CA); % top area
    Area_T_train = (W_dist + L_MT + W_eff + 2 * CA + d * SL) * (W_N_trains + 2 * CA + d * SL); % bottom area
    ...
    Area_T_P = (W_PB + W_BB + 2 * CA + d * SL) * (W_N_trains + 2 * CA + d * SL);
    ...
    % Stainless
    M_Steel_kg = (M_SS_LIFT + M_SS_blower + M_SS_GBT);
    ...
    COD_water = S /10^3 * V_treated; % [kg-COD]
    ...
    RSR = Q_ras/Q_LD; %input('Enter the Return Sludge Ratio to Average Wastewater Flow: ');
    Qavg=Q_mgd(i);
    ...
    %% Inventory Analysis
    if i==1
        output_CAS.(current_system).LCC=output1_LCC;
    else
        output_CAS.(current_system).LCC=[output_CAS.(current_system).LCC; output1_LCC];
    end
    ...
    if i==1
        output_CAS.(current_system).Concrete=all_con(2,:);
        output_CAS.(current_system).Steel=array2table(all_con{3,:}+all_con{9,:},'VariableNames',all_VarNames);
        output_CAS.(current_system).Excavation=all_con(19,:);
        output_CAS.(current_system).Construction_Misc=array2table(sum(all_con{4:8,:})+sum(all_con{10:18,:})+sum(all_con{20:38,:}),'VariableNames',all_VarNames);
        output_CAS.(current_system).Lift_Pumping=array2table(Power_pct(1)*sum(all_op{10:16,:}),'VariableNames',all_VarNames);
        output_CAS.(current_system).Gas_Sparging=array2table(Power_pct(2)*sum(all_op{10:16,:}),'VariableNames',all_VarNames);
        output_CAS.(current_system).Gravity_Belt_Thickeners=array2table(Power_pct(3)*sum(all_op{10:16,:}),'VariableNames',all_VarNames);
        output_CAS.(current_system).Operation_Misc=array2table(sum(all_op{1:9,:}),'VariableNames',all_VarNames);
    else
        output_CAS.(current_system).Concrete=[output_CAS.(current_system).Concrete; all_con(2,:)];
        output_CAS.(current_system).Steel=[output_CAS.(current_system).Steel;array2table(all_con{3,:}+all_con{9,:},'VariableNames',all_VarNames)];
        output_CAS.(current_system).Excavation=[output_CAS.(current_system).Excavation;all_con(19,:)];
        output_CAS.(current_system).Construction_Misc=[output_CAS.(current_system).Construction_Misc;array2table(sum(all_con{4:8,:})+sum(all_con{10:18,:})+sum(all_con{20:38,:}),'VariableNames',all_VarNames)];
        output_CAS.(current_system).Lift_Pumping=[output_CAS.(current_system).Lift_Pumping;array2table(Power_pct(1)*sum(all_op{10:16,:}),'VariableNames',all_VarNames)];
        output_CAS.(current_system).Gas_Sparging=[output_CAS.(current_system).Gas_Sparging;array2table(Power_pct(2)*sum(all_op{10:16,:}),'VariableNames',all_VarNames)];
        output_CAS.(current_system).Gravity_Belt_Thickeners=[output_CAS.(current_system).Gravity_Belt_Thickeners;array2table(Power_pct(3)*sum(all_op{10:16,:}),'VariableNames',all_VarNames)];
        output_CAS.(current_system).Operation_Misc=[output_CAS.(current_system).Operation_Misc;array2table(sum(all_op{1:9,:}),'VariableNames',all_VarNames)];
    end
    
    % Direct emission
    if i==1
        output_CAS.(current_system).Direct_Emissions=array2table(sum(IMPACT_DE{1:8,:}),'VariableNames',all_VarNames);
    else
        output_CAS.(current_system).Direct_Emissions=[output_CAS.(current_system).Direct_Emissions;array2table(sum(IMPACT_DE{1:8,:}),'VariableNames',all_VarNames)];
    end
    
    % Avoided Energy
    if i==1
        output_CAS.(current_system).Avoided_Energy=array2table(sum(all_avoid{1:7,:}),'VariableNames',all_VarNames);
    else
        output_CAS.(current_system).Avoided_Energy=[output_CAS.(current_system).Avoided_Energy;array2table(sum(all_avoid{1:7,:}),'VariableNames',all_VarNames)];
    end
    ...
    if i==trials
        names=fieldnames(output_CAS.(current_system));
        for j=1:length(names)
            if isstruct(output_CAS.(current_system).(names{j}))==1
                output_CAS.(current_system).(names{j})=struct2table(output_CAS.(current_system).(names{j}));
                %         else
...
%% Calculations
designs=fieldnames(output_CAS);
output_CAS.LCC_LCA=table();
output_CAS.LCA=table();
for i=1:length(designs)
    categories=fieldnames(output_CAS.(designs{i}));
    for j=1:length(categories)
        if j==1
            output_CAS.LCC_LCA=[output_CAS.LCC_LCA; varfun(@mean,output_CAS.(designs{i}).(categories{j})) varfun(@std,output_CAS.(designs{i}).(categories{j})) varfun(@median,output_CAS.(designs{i}).(categories{j}))]; %Calculates average and standard dev. and median from all trials
            output_CAS.(designs{i}).LCA_Avg=table();
        else
            output_CAS.(designs{i}).LCA_Avg=[output_CAS.(designs{i}).LCA_Avg; varfun(@mean,output_CAS.(designs{i}).(categories{j})) varfun(@var,output_CAS.(designs{i}).(categories{j})) varfun(@median,output_CAS.(designs{i}).(categories{j}))]; %Calculates average, variance, and median of impacts from all trials
        end
    end
    output_CAS.(designs{i}).LCA_Avg.Properties.RowNames=categories(2:length(categories));
    output_CAS.(designs{i}).LCA_sum=table();
    output_CAS.(designs{i}).LCA_sum=varfun(@sum,output_CAS.(designs{i}).LCA_Avg); %Finds the sum of means and variances for each impact category for a given design
    output_CAS.(designs{i}).LCA_sum{1,10:18}=output_CAS.(designs{i}).LCA_sum{1,10:18}.^2; %Squares variance to calculate standard dev.
    output_CAS.(designs{i}).LCA_Const=varfun(@sum,output_CAS.(designs{i}).LCA_Avg(1:4,:));
    output_CAS.(designs{i}).LCA_Const.Properties.VariableNames=Const_names;
    output_CAS.(designs{i}).LCA_OM=varfun(@sum,output_CAS.(designs{i}).LCA_Avg(5:9,:));
    output_CAS.(designs{i}).LCA_OM.Properties.VariableNames=OM_names;
    output_CAS.LCA=[output_CAS.LCA; output_CAS.(designs{i}).LCA_sum output_CAS.(designs{i}).LCA_Const output_CAS.(designs{i}).LCA_OM];
end
output_CAS.LCC_LCA.Properties.RowNames=designs;
output_CAS.LCC_LCA=[output_CAS.LCC_LCA output_CAS.LCA];
close(wait)